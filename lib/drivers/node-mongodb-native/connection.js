...
NativeConnection.prototype.doOpen = function (fn) {
  ...
  //TODO: need to define Path object to server as replacement for Db
  this.path = new Path(this.name, this.options.path);

  var self = this;
  //TODO: fs.open opens a file.  A directory does not need to be opened, but its existence should be confirmed.
  this.fs.stat(this.path, function(err, stats){
    if (err) return fn(err);
    if (!stats.isDirectory()) return fn("Path is not a valid directory");
    if (options.permissions){
      var permissions = '0' + (stats.mode & parseInt('777', 8)).tpoString(8);
      if (permissions !== options.permissions) return fn("invalid permissions for path");
      listen(self);
      fn();
    }
  });
...
NativeConnection.prototype.usePath = function (name) {
  // we have to manually copy all of the attributes...
  var newConn = new this.constructor();
  newConn.name = name;
  newConn.base = this.base;
  ...
  if (this.path && this.path._state == 'connected') {
    wireup();
  } else {
    this.once('connected', wireup);
  }
  ...
  function wireup () {
    newConn.path = self.path.path(name);
    newConn.onOpen();
    // setup the events appropriately
    listen(newConn);
  }
  ...
  // push onto the otherDbs stack, this is used when state changes
  this.otherPaths.push(newConn);
  newConn.otherPaths.push(this);
...
NativeConnection.prototype.doOpenSet = function (fn) {
  ...
  //TODO: Do we need a 'fullsetup' event for the Path object?
